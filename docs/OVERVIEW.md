...
The workspace root for the nine services and seven apps that make up 241.
**This directory is not a git repository.** Each service below is its own repository
with its own remote, and nothing at this level is ever pushed.

That has one consequence worth internalising before doing anything here:
**anything a service needs must live inside that service's folder.** Docs,
skills, and the constitution are duplicated into the services on purpose —
there is no shared parent to inherit from.
...
on `postgres:5432`; `infra/postgres/init-databases.sql` creates eight of them
...
Two things stayed `teacher` on purpose:

- the `TEACHER` role code, in identity-service and every app that checks it;
- `packages/*/features/profile`, in all seven web apps, which parses a
  `/profiles/:id` response shape with `teacher` and `student` branches that
  identity-service does not serve. That code is dead and needs rewriting, not
  renaming — see the audit section.
...
Read each app's own `CLAUDE.md` before working on it — they differ in what they
kept, what they call, and what is still broken. Two are worth knowing about from
here:

- **`portal-web` renders CMS content with unsanitised `v-html`** on three
  public pages. The rule is switched off for exactly those three files, so a
  fourth fails the build. Not resolved.
- **`/settings` has no owner.** `platform/settings`'s `AppSetting` — logo,
  title, maintenance mode, per-item menu visibility — was an admin screen that
  did not survive the split. `academic-web`, `admin-web` and
  `assessment-web` deleted the feature; the other four keep it and list the
  prefix in `UNROUTED_PREFIXES`.
...
**One gateway serves all seven.** `infra/nginx/generate.mjs` reads every app's
manifest and emits one `server` block per app, keyed on `server_name`, sharing
...
`docs/OVERVIEW.md` is this file, and it is the one that ships. `CLAUDE.md`
beside it is a local-only stub that imports it — the content lives in exactly
...
`.specify/memory/constitution.md` here holds the same constitution plus all six
profiles gathered together, because work at this level spans services.
...
**`infra/nginx/*.conf` are generated — never hand-edit them.** The gateway's
routing table has one source per app — each app's own
`api-routes.config.ts` — and the two configs are rendered from all six:

```bash
node infra/nginx/generate.mjs            # rewrite both configs
node infra/nginx/generate.mjs --check    # exit 1 if either is stale
```

Add an endpoint prefix to an app's manifest, regenerate, and both dev (Vite
proxy) and production (gateway) learn it at once. A prefix a frontend calls but
nothing routes is answered with `index.html` at HTTP 200 in dev and refused in
production — `UNROUTED_PREFIXES` exists to make both 404. Run `--check` before
a deploy; run `pnpm run smoke:gateway` in `academic-web` against the deployed
origin after one.

Adding an app is: write its `api-routes.config.ts`, add an entry to `APPS` in
the generator, mount its `dist` in `docker-compose.prod.yml`. Adding a service
an app calls is one entry in `SERVICES` in the same file. No list is written
twice — the generator exits 1 naming the app if a manifest routes a prefix to
a service with no upstream declared.
...
`docker-compose.prod.yml`, and has a `Dockerfile` (it had none).
...
`.claude/skills/` at this root carries the ten speckit skills plus the same six
engineering skills each service has: `clean-architecture`, `clean-code`,
`nestjs-best-practices`, `domain-driven-design`, `code-review`,
`diagnosing-bugs`. Each service's copy is scoped to that service's folder, so the
right one loads automatically when you work inside it. `skills-lock.json` pins
their sources.

Speckit here is version 1.0.4.dev0.

## Versioning and releases

Per folder, never per repository — `VERSIONING.md` has the whole of it. The
short form:

```bash
node scripts/release.mjs <folder> <patch|minor|major> [--dry-run]
```

Bumps that folder's `package.json`, writes its `CHANGELOG.md` from the commits
since its last tag that touched it, and tags `<folder>@<version>`. Production
deploys from **tags**, not from a branch; `main` is the only long-lived branch.
...
Not Changesets: it needs a root workspace, and every folder here keeps its own
`pnpm-lock.yaml` (each `Dockerfile` installs `--frozen-lockfile` from it) and
its own `overrides` pinning advisories scoped to that service's tree.

## CI

`.github/workflows/validate.yml` runs on every pull request and every push to
`main`. It is the same six commands you run by hand, per folder:

```
format:check → lint → typecheck → lint:strict → test → build
```

Separate steps rather than `pnpm run validate`, so the red step names the
problem instead of one exit code hiding it.

**Only the folders you touched run.** `.github/scripts/changed-folders.mjs`
diffs against the merge base and emits the matrix. It fails open: an
unreadable base — shallow clone, force-push, first commit — validates all
sixteen rather than silently skipping. Touching `.github/` also validates all
sixteen, because a change there can break any of them.

Two things beyond the per-folder commands:

- **`prisma generate` before the checks** in the nine services, with the same
  throwaway `DATABASE_URL` the Dockerfile uses. Nothing in CI connects to a
  database.
- **`node infra/nginx/generate.mjs --check`** whenever `infra/nginx/` or any
  `api-routes.config.ts` changes. A manifest edited without regenerating is a
  gateway that disagrees with its app, and that only shows up in production.

**Protect `main` with the single `validated` check**, not the sixteen matrix
jobs — their names come and go with the matrix, and a required check that does
not exist blocks every merge. A skipped job counts as a pass there: it means
the change did not touch that folder.
...
identity-service 251, inventory-service 103, `node infra/nginx/generate.mjs --check` clean,
...
Nine services, nine images, nine databases. Only `nginx` declares `depends_on`
in `docker-compose.prod.yml`, and every service has its own healthcheck, so
rebuilding one leaves the other eight running untouched:

```bash
docker compose -f docker-compose.prod.yml up -d --build hr-service
```
...
`infra/nginx/nginx.conf` declared plain static upstreams
...
`node infra/nginx/generate.mjs --check` could never have caught it: it proves
the file is in sync with the manifests, not that nginx accepts it. CI now runs
both:

```yaml
- run: node infra/nginx/generate.mjs --check
- run: docker run --rm -v "$PWD/infra/nginx/nginx.conf:/etc/nginx/conf.d/default.conf:ro" nginx:1.27-alpine nginx -t
```
